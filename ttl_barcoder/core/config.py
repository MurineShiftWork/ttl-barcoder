...
class BarcodeConfigModel(BaseModel):
    ...
    barcode_bits: int = Field(default=37, ge=16, le=64, 
                             description="Number of bits in barcode (16-64)")
    time_precision_ms: float = Field(default=10.0, gt=0, le=1000,
                                    description="Timestamp precision in ms")
    bit_duration_ms: float = Field(default=35.0, gt=0, le=1000,
                                  description="Duration of each bit pulse in ms")
    init_duration_ms: float = Field(default=10.0, gt=0, le=100,
                                   description="Duration of init pulses in ms")
    tolerance: float = Field(default=0.25, ge=0.05, le=0.5,
                           description="Timing tolerance (0.05-0.5)")
    
    @validator('bit_duration_ms')
    def bit_duration_must_be_reasonable(cls, v, values):
        if 'tolerance' in values:
            min_safe = 10.0 / values['tolerance']  # 10ms jitter safety
            if v < min_safe:
                raise ValueError(f'bit_duration_ms should be ≥{min_safe:.1f}ms for reliable decoding')
        return v
...
class BarcodeConfig:
    ...
    def validate(self) -> bool:
        ...
        if PYDANTIC_AVAILABLE:
            ...
            # Basic validation
            return (16 <= self.barcode_bits <= 64 and 
                   self.time_precision_ms > 0 and
                   self.bit_duration_ms > 0 and
                   self.init_duration_ms > 0 and
                   0.05 <= self.tolerance <= 0.5)
    ...
    def __str__(self) -> str:
        ...
        return (f"BarcodeConfig({self.barcode_bits}-bit, "
